...
import { BarChart2, Coins, User, Briefcase, Trophy, MessageSquare, HelpCircle, Gift, Rocket, X, GraduationCap, FileText, Activity, Globe, DollarSign, Lock, ChevronRight, Search, MessageCircle, Users, Headphones, BookOpen, ThumbsUp, Grid3x3, Send, Facebook, Twitter, Youtube, Instagram, Music, LogIn, UserPlus } from 'lucide-react';
import IconTab from './IconTab';
import { useLocation, useNavigate } from 'react-router-dom';

function LeftSidebar({ isOpen, onClose, activeMenu, setActiveMenu, setShowLeftSidebar, setShowSignals, setShowSocialModal }) {
  ...
  const tradingSubMenu = [
    { icon: Coins, label: 'Quick Trading Real Account', route: '/registration' },
    { icon: GraduationCap, label: 'Quick Trading Demo Account', route: '/registration' },
    { icon: FileText, label: 'Shares Trading Real Account', route: '/registration' },
    { icon: FileText, label: 'Shares Trading Demo Account', route: '/registration' },
  ...
  const financeSubMenu = [
    { icon: Coins, label: 'Deposite', route: '/finance/Deposite', active: location.pathname === '/finance/Deposite' },
    { icon: DollarSign, label: 'Withdrawal', route: '/finance/Withdrawal', active: location.pathname === '/finance/Withdrawal' },
    { icon: DollarSign, label: 'History', route: '/finance/History', active: location.pathname === '/finance/History' },
    { icon: DollarSign, label: 'Cashback', route: '/finance/Cashback', active: location.pathname === '/finance/Cashback' },
    { icon: DollarSign, label: 'Promo codes', route: '/finance/Promo-codes', active: location.pathname === '/finance/Promo-codes' },
    { icon: DollarSign, label: 'My Safe', route: '/finance/My-Safe', active: location.pathname === '/finance/My-Safe' },
  ];

  const profileSubMenu = [
    { icon: User, label: 'Trading Profile ', route: '/profile/Trading-Profile', active: location.pathname === '/profile/Trading-Profile' },
    { icon: Lock, label: 'Profile edit', route: '/profile/Profile-edit', active: location.pathname === '/profile/Profile-edit' },
    { icon: Lock, label: 'Loyalty Program', route: '/profile/Loyalty-Program', active: location.pathname === '/profile/Loyalty-Program' },
    { icon: Lock, label: 'Security', route: '/profile/Security', active: location.pathname === '/profile/Security' },
    { icon: Lock, label: 'Trading History', route: '/profile/Trading-History', active: location.pathname === '/profile/Trading-History' },
    { icon: Lock, label: 'Pocket frends', route: '/profile/Pocket-frends', active: location.pathname === '/profile/Pocket-frends' },
  ];

  const marketSubMenu = [
    { icon: Briefcase, label: 'Stock Market', route: '/market/Stock-Market', active: location.pathname === '/market/Stock-Market' },
    { icon: Globe, label: 'Purchases & Gifts', route: '/market/Purchases-Gifts', active: location.pathname === '/market/Purchases-Gifts' },
    { icon: Globe, label: 'Gem Lottery', route: '/market/Gem-Lottery', active: location.pathname === '/market/Gem-Lottery' },
    { icon: Globe, label: 'Social rewards', route: '/market/Social-rewards', active: location.pathname === '/market/Social-rewards' },
    { icon: Globe, label: 'Gems mining', route: '/market/Gems-mining', active: location.pathname === '/market/Gems-mining' },
  ];

  const achievementsSubMenu = [
    { icon: Trophy, label: 'Achievement', route: '/achievements/Achievement', active: location.pathname === '/achievements/Achievement' },
    { icon: Trophy, label: 'History', route: '/achievements/History', active: location.pathname === '/achievements/History' },
    { icon: Trophy, label: 'Rating', route: '/achievements/Rating', active: location.pathname === '/achievements/Rating' },
    { icon: Trophy, label: 'Community help', route: '/achievements/Community-help', active: location.pathname === '/achievements/Community-help' },
  ];
    ...
    if (menu === 'Chat') {
      return (
        <div
          ...
          } md:static md:w-80 md:flex md:flex-col md:${activeMenu === menu ? 'block' : 'hidden'}`}
        >
          <div className="flex items-center justify-between p-4 border-b border-zinc-800">
            ...
            <button onClick={() => setActiveMenu(null)} className="text-zinc-400 hover:text-white ml-2">
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="flex items-start gap-3 p-3 bg-zinc-900/50 rounded-lg cursor-pointer hover:bg-zinc-800/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-medium truncate">Support Chat (Online)</h3>
                  <span className="text-xs text-zinc-400 ml-2">26 Sep, 15:09</span>
                </div>
                <p className="text-sm text-zinc-300 truncate mt-1">Welcome to the Support Chat! Here you can...</p>
            ...
            <div className="flex items-start gap-3 p-3 bg-zinc-900/50 rounded-lg cursor-pointer hover:bg-zinc-800/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <h3 className="text-white font-medium truncate">EN General chat (English)</h3>
                  <span className="text-xs text-zinc-400 ml-2">12:26</span>
                </div>
                <p className="text-sm text-zinc-300 truncate mt-1">Go to profile section, complete account v.13053</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <ul className="flex flex-col h-full gap-0">
            ...
            <li>
              <div
                ...
                onClick={() => {
                  console.log('Support Service clicked');
            ...
            <li>
              <div
                className="flex items-center gap-3 px-4 py-3 text-zinc-200 hover:bg-indigo-900/50 transition-all duration-200 border-b border-zinc-800"
                onClick={() => {
                  console.log('Guides and Tutorials clicked');
            ...
            <li className="px-4 py-3 border-b border-zinc-800">
              <div className="text-white font-medium mb-3">Official channels:</div>
            ...
            <li key={index}>
              <div
                className={`flex items-center gap-3 px-4 py-3 text-zinc-200 hover:bg-indigo-900/50 transition-all duration-200 border border-zinc-800 rounded ${
                  item.active || location.pathname === item.route ? 'text-white bg-indigo-900/50' : 'bg-[#0a0e18]'
                }`}
                onClick={() => {
              ...
              >
                ...
                {(item.active || location.pathname === item.route) && <ChevronRight size={16} className="text-white" />}
              </div>
            </li>
          ))}
        </ul>
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-14 left-0 h-[calc(100vh-3.5rem)] ${isOpen ? 'w-64' : 'w-20'} bg-[#0a0e18] border-r border-zinc-800/50 flex flex-col items-center py-4 gap-2 z-60 shadow-lg transition-all duration-300 ${
  ...
  >
    <UserPlus size={16} />
    <span className={`${isOpen ? 'inline ml-1' : 'hidden'}`}>Signup</span> {/* Added ml-1 for spacing */}
  </button>
  ...
  >
    <LogIn size={16} />
    <span className={`${isOpen ? 'inline ml-1' : 'hidden'}`}>Login</span> {/* Added ml-1 for spacing */}
  </button>
  ...
  >
    <Rocket size={16} />
    <span className={`${isOpen ? 'inline ml-1' : 'hidden'}`}>Upgrade</span>
  </button>
</div>
      </aside>
